...
// Measure Mode
 ...
 //20M
 #define		MAXLMT_20M		0x40400000				// 3.0
 #define		MINLMT_20M		0x40000000				// 2.0
 #define		CHGCOEF_20M		0xB9400000				// -0.00018
 #define		MINLMT_MOV_20M	0x00000000				// 0.0
 #define		CHGCOEF_MOV_20M	0xB8800000				// -0.00006104
 //13M
 #define		MAXLMT_13M		0x40333333				// 2.8
 #define		MINLMT_13M		0x3FC51EB9				// 1.54
 #define		CHGCOEF_13M		0xBA120820				// -0.00056
 #define		MINLMT_MOV_13M	0x00000000				// 0.0
 #define		CHGCOEF_MOV_13M	0xB92B6DB7				// -0.00016 
...
#define		COEFTBL	7
const unsigned long	ClDiCof_20M[ COEFTBL ]	= {
		DIFIL_S2_20M,		/* 0 */
		DIFIL_S2_20M,		/* 1 */
		DIFIL_S2_20M,		/* 2 */
		DIFIL_S2_20M,		/* 3 */
...
const unsigned long	ClDiCof_13M[ COEFTBL ]	= {
		DIFIL_S2_13M,		/* 0 */
		DIFIL_S2_13M,		/* 1 */
		DIFIL_S2_13M,		/* 2 */
		DIFIL_S2_13M,		/* 3 */
...
//#ifdef	AF_MID_MOUNT
	UsFinSts	= (unsigned short)( UcHlxSts - EXE_END ) + (unsigned short)( UcHlySts - EXE_END ) + 
				  (unsigned short)( UcAtxSts - EXE_END ) + (unsigned short)( UcAtySts - EXE_END ) + 
				  ( UsOscSts - (unsigned short)EXE_END ) + ( UsAfOftSts - (unsigned short)EXE_END ) + 
...
void	MesFil( unsigned char	UcMesMod )
{
	...
	if( !UcMesMod ) {								// Hall Bias&Offset Adjust
		...
		RamWrite32A( mes1ad, 0x00000000 ) ;		// 0x10F3
		RamWrite32A( mes1ae, 0x00000000 ) ;		// 0x10F4
		RamWrite32A( mes1ba, 0x3F7FD400 ) ;		// 0x10F5	HPF5Hz
		RamWrite32A( mes1bb, 0xBF7FD400 ) ;		// 0x10F6
		RamWrite32A( mes1bc, 0x3F7FA840 ) ;		// 0x10F7
		RamWrite32A( mes1bd, 0x00000000 ) ;		// 0x10F8
		RamWrite32A( mes1be, 0x00000000 ) ;		// 0x10F9
		...
		RamWrite32A( mes2ad, 0x00000000 ) ;		// 0x11F3
		RamWrite32A( mes2ae, 0x00000000 ) ;		// 0x11F4
		RamWrite32A( mes2ba, 0x3F7FD400 ) ;		// 0x11F5	HPF5Hz
		RamWrite32A( mes2bb, 0xBF7FD400 ) ;		// 0x11F6
		RamWrite32A( mes2bc, 0x3F7FA840 ) ;		// 0x11F7
		RamWrite32A( mes2bd, 0x00000000 ) ;		// 0x11F8
		RamWrite32A( mes2be, 0x00000000 ) ;		// 0x11F9
...
void	LopSin( unsigned char	UcDirSel, unsigned char	UcSonOff )
{
	...
	if( UcSonOff ) {
		...
		if( !UcDirSel ) {
			...
			UcEqSwY &= ~EQSINSW ;
			if( UcModule == MODULE_20M ){	//20M
				RamWrite32A( sxsin, 0x3C676E00 ) ;			// 0x10D5		-37dB
			}
			else{							//13M
				RamWrite32A( sxsin, 0x3BB84480 ) ;			// 0x10D5		-45dB
	  		}
		} else {
			...
			UcEqSwY |= 0x10 ;
			if( UcModule == MODULE_20M ){	//20M
				RamWrite32A( sysin, 0x3C676E00 ) ;			// 0x11D5		-37dB
			}
			else{							//13M
				RamWrite32A( sysin, 0x3BB84480 ) ;			// 0x10D5		-45dB
	  		}
		}
...
{
	unsigned long	DIFIL_S2;
	
	if( UcModule == MODULE_20M ){
		DIFIL_S2	= DIFIL_S2_20M;
	}
	else{
		DIFIL_S2	= DIFIL_S2_13M;
	}
...
void StbOnn( void )
{
	...
	if( (( UcRegValx & 0x01 ) != 0x01 ) && (( UcRegValy & 0x01 ) != 0x01 ))
	{
		...
		if (UcCnt == 60){
			RamWriteA(SXOFFZ2 , 0);	// 0x1462	Offset 0 clear
			RamWriteA(SYOFFZ2 , 0);	// 0x14E2	Offset 0 clear
		}
	}
	else
	{
...
void StbOnnN( unsigned char UcStbY , unsigned char UcStbX )
{
	...
	for (UcCnt = 0; UcCnt < 60; UcCnt++) {
		RegReadA( RH_SMTSRVSTT,	&UcRegIni ) ;		// 0x01F8		Smooth Servo phase read
		if ((UcRegIni & UcSttMsk) == ( 0x66 & UcSttMsk )) break;
		WitTim( WTLE );
	}
...
unsigned short	OscAdjA( unsigned short UsTgtCnt )
{
	...
	RegReadA( SRVDIV, &UcSrvDivBk ) ;			/* 0x0211 	Srv Div back UP */
	RegWriteA( SRVDIV,	0x00 ) ;				// 0x0211	 SRV Clock = Xtalck
	RegWriteA( OSCSET, OSCSET_INI ) ;			// 0x0257	 
 #ifdef	OSC_EXCLK
	RegReadA( PWMDIV, &UcPwmDivBk ) ;			/* 0x0210 	Pwm Div back UP */
	RegReadA( CLKSEL, &UcClkSelBk ) ;			/* 0x020C 	ClkSel back UP */
	RegWriteA( PWMDIV, 0x00 ) ;					/* 0x0210 	Pwm Div 1/1 */
	RegWriteA( CLKSEL, UcClkSelBk | 0x01 ) ;	/* 0x020C 	ClkSel re-store */
	UcOscMod = OSCMODE_EXC ;
 #else
	UcOscMod = OSCMODE_I2C ;
 #endif
	RegWriteA( OSCMAMODE, UcOscMod ) ;								// 0x0265
	RegWriteA( OSCTCNTL, (unsigned char)UsTgtCnt ) ;				// 0x0266
	RegWriteA( OSCTCNTH, (unsigned char)( UsTgtCnt >> 8 ) ) ;		// 0x0267
	UcClkJdg = 0 ;
	
	while( UcClkJdg == 0 )
	{
		RegWriteA( OSCCNTEN, 0x01 ) ;		// 0x0258	 Trigger ON
		RegWriteA( OSCCNTEN, 0x00 ) ;		// 0x0258	 Trigger OFF
		RegReadA(  OSCMAMODE, &UcClkJdg ) ;	// 0x0265	 Status Read
		UcClkJdg &= 0x03;
	}
	
	UsResult = EXE_END ;
	
	RegReadA( OSCSET, &UcOscSetVal ) ;		// 0x0257	 
	StAdjPar.UcOscVal = UcOscSetVal ;
	RegWriteA( OSCMAMODE, OSCMODE_DEF ) ;	// 0x0265
	...
	RegWriteA( SRVDIV,	UcSrvDivBk ) ;		// 0x0211	 SRV Clock set
 #ifdef	OSC_EXCLK
	RegWriteA( PWMDIV, UcPwmDivBk ) ;		/* 0x0210 	Pwm Div re-store */
	RegWriteA( CLKSEL, UcClkSelBk ) ;		/* 0x020C 	ClkSel re-store */
 #endif
	return( UsResult );
}
...
void	SetGcf( unsigned char	UcSetNum )
{
	...
	if( UcModule == MODULE_20M ){
		UlH1Coefval	= ClDiCof_20M[ UcSetNum ] ;
	}
	else{
		UlH1Coefval	= ClDiCof_13M[ UcSetNum ] ;
	}
...
{
	unsigned long	MAXLMT;
	unsigned long	MINLMT;
	unsigned long	CHGCOEF;
	unsigned long	MINLMT_MOV;
	unsigned long	CHGCOEF_MOV;
	
	if( UcModule == MODULE_20M ){
		MAXLMT		= MAXLMT_20M;
		MINLMT		= MINLMT_20M;
		CHGCOEF		= CHGCOEF_20M;
		MINLMT_MOV	= MINLMT_MOV_20M;
		CHGCOEF_MOV	=CHGCOEF_MOV_20M;
	}
	else{
		MAXLMT		= MAXLMT_13M;
		MINLMT		= MINLMT_13M;
		CHGCOEF		= CHGCOEF_13M;
		MINLMT_MOV	= MINLMT_MOV_13M;
		CHGCOEF_MOV	=CHGCOEF_MOV_13M;
	}
	...
	case ( ACTMODE ):				// initial 
		...
		/* Zoom Step */
		if( UcModule == MODULE_20M ){
			UlH1Coefval	= ClDiCof_20M[ 0 ] ;
		}
		else{
			UlH1Coefval	= ClDiCof_13M[ 0 ] ;
		}
...
//#ifdef	AF_MID_MOUNT
//********************************************************************************
// Function Name 	: AfMidOffAdj
// Retun Value		: NON
// Argment Value	: NON
// Explanation		: 
// History			: First edition 						2014.06.26 T.Tokoro
//********************************************************************************
//#define AFMID_DEBUG_LOG
//#define AFMID_TEST
#ifndef	AFMID_TEST
	#define	AFMID_MESPOS		0x03b8		// 0x03b8 --> 0x7700	Measure position
	#define	AFMID_OPOFSET	0x3f434f78	// 0.762931
	#define	AFMID_TIM			250
#else	//AFMID_TEST
	#define	AFMID_TIM			2000
#endif	//AFMID_TEST
#define	AFMID_ADJPOS		0x0400		// 0x0400 --> 0x8000	Adjust position : current = 0[mA]

#if 0
unsigned short	AfMidOffAdj( void )
{
	unsigned char	ucRegDat, ucDRVFC3AF, ucDRVFC4AF;
	unsigned short	usErrFlg;
	char			cAdjOft;
	short			sMESPOS;
	UnFltVal		UnMesMin, UnMesMax, UnAdjMin, UnAdjMax, UnAdjRes, UnStep, UnAdjOft, UnMesOff;

#ifndef	AFMID_TEST
	unsigned char	ucSTBB0, ucOPGSEL2, ucVGA_SW1, ucDOFSTDAF;
	unsigned short	usTREG_HL;
	unsigned long	ulDAZHLO;
#else	//AFMID_TEST
	unsigned char	ucCnt;
	short			sDir;
	UnFltVal		UnMesPls, UnMesMns, UnMesOff1, UnMesOff2;
#endif	//AFMID_TEST
	
#ifdef	AFMID_DEBUG_LOG
	unsigned char	ucCon, ucOff;
#endif	//AFMID_DEBUG_LOG
	
	usErrFlg = EXE_END;									// Error Flg Clear
...
#ifndef	AFMID_TEST
	RegReadA( STBB0,	&ucSTBB0 );						// 0x0250
	RegReadA( OPGSEL2,	&ucOPGSEL2 );					// 0x0255
	RegReadA( VGA_SW1,	&ucVGA_SW1 );					// 0x025B
	RamRead32A( DAZHLO, &ulDAZHLO );					// 0x1529
	ucDOFSTDAF = GetDOFSTDAF();							// DOFSTDAF 0x0083 0x0084
	RamReadA( TREG_H,	&usTREG_HL ) ;					// TREG_H(0x0380) - TREG_L(0x0381)
#endif	//AFMID_TEST
...
#ifndef	AFMID_TEST
	sMESPOS = AFMID_MESPOS;
	UnMesOff.UlLngVal = AFMID_OPOFSET;
#else	//AFMID_TEST
	
	RegWriteA( STBB0,		0x83 ) ;					// 0x0250	AF_Drv=ON, DAC_V=ON, ADC=ON
	RegWriteA( VGA_SW1,		0x04 ) ;					// 0x025B	OP_SWAF3=off, OP_SWAF2=on, OP_SWAF1=off, OP_SWAF0=off

////////// ////////// ////////// ////////// //////////
// Direction.
////////// ////////// ////////// ////////// //////////
	SetDOFSTDAF_WT( 0x00 );								// 0x0084	min offset
	

	
	SetTregAf( 0x0400 + 0x0100 );						// TREG_H(0x0380) - TREG_L(0x0381) mesure position
	WitTim( AFMID_TIM );								// 
	UnMesPls.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)

	
	SetTregAf( 0x0400 - 0x0100 );						// TREG_H(0x0380) - TREG_L(0x0381) mesure position
	WitTim( AFMID_TIM );								// 
	UnMesMns.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)

	
	if( UnMesPls.SfFltVal < UnMesMns.SfFltVal){
		sDir = +1;
	}
	else{
		sDir = -1;
...
////////// ////////// ////////// ////////// //////////
	SetDOFSTDAF_WT( 0x1F );								// 0x0084	max offset
	

	
	sMESPOS = 0x0400 + ( sDir * 16 * 0x08 );
	for( ucCnt = 0; ucCnt < 16 ; ucCnt++ ){
		SetTregAf( 0x0400 + ( sDir * ucCnt * 0x08 ) );	// TREG_H(0x0380) - TREG_L(0x0381) mesure position
		WitTim( AFMID_TIM );							// 
		UnMesOff.UlLngVal = MesMSABS1AV();				// 1024 Times Measure average : AD4Z(0x144F)

		if( UnMesOff.SfFltVal < 0.92 ){
			sMESPOS = 0x0400 + ( sDir * ucCnt * 0x08 );
...
////////// ////////// ////////// ////////// //////////
	...
	SetTregAf( sMESPOS );								// TREG_H(0x0380) - TREG_L(0x0381) mesure position
	WitTim( AFMID_TIM );								// 
	UnMesOff.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)

#endif	//AFMID_TEST
	
	RegWriteA( STBB0,		0xA3 ) ;					// 0x0250	AF_Drv=ON, AF_Op=ON, DAC_V=ON, ADC=ON
	RegWriteA( VGA_SW1,		0x04 ) ;					// 0x025B	OP_SWAF3=off, OP_SWAF2=on, OP_SWAF1=off, OP_SWAF0=off
	RegWriteA( OPGSEL2,		0x00 ) ;					// 0x0255	x6
	RamWrite32A( DAZHLO, 	UnMesOff.UlLngVal );		// 0x1529	VGAAF Offset

	
#ifdef	AFMID_TEST
	SetDOFSTDAF_WT( 0x1F );								// 0x0084	min offset
	
	SetTregAf( AFMID_ADJPOS );							// TREG_H(0x0380) - TREG_L(0x0381) Adjust position
	WitTim( AFMID_TIM );								// 
	
	UnMesOff1.UlLngVal = UnMesOff.UlLngVal;
	
	for( ucCnt = 0; ucCnt < 16 ; ucCnt++ ){
		UnMesOff2.UlLngVal = MesMSABS1AV();				// 1024 Times Measure average : AD4Z(0x144F)

		if( UnMesOff2.SfFltVal < -0.9f ){
			UnMesOff1.SfFltVal += 0.01f;
		}
		else{
			break;
		}
		RamWrite32A( DAZHLO, 	UnMesOff1.UlLngVal );	// 0x1529	VGAAF Offset
...
////////// ////////// ////////// ////////// //////////
	...
	UnMesMax.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)
	

	
////////// ////////// ////////// ////////// //////////
// measuring minimum current.
////////// ////////// ////////// ////////// //////////
	
	SetDOFSTDAF_WT( 0x1F );								// 0x0084	max offset
	
	UnMesMin.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)
	

	
////////// ////////// ////////// ////////// //////////
// calculating step of adjustment.
////////// ////////// ////////// ////////// //////////
	
	UnStep.SfFltVal = ( UnMesMin.SfFltVal - UnMesMax.SfFltVal ) / 31.0f;
	

	
////////// ////////// ////////// ////////// //////////
// move AF to free position ( current is 0[mA] ).
////////// ////////// ////////// ////////// //////////
...
////////// ////////// ////////// ////////// //////////
	...
	UnAdjMax.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)
...
////////// ////////// ////////// ////////// //////////
	...
	UnAdjMin.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)
	

	
////////// ////////// ////////// ////////// //////////
// calculating the offset to let the current become 0mA.
////////// ////////// ////////// ////////// //////////
	
	UnAdjOft.SfFltVal = ( UnAdjMin.SfFltVal - UnAdjMax.SfFltVal ) / UnStep.SfFltVal;
	

	
	cAdjOft = (char)(UnAdjOft.SfFltVal + 0.5f);
	

	
////////// ////////// ////////// ////////// //////////
// setting adjusted value of offset.
// measuring current with adjusted offset.
////////// ////////// ////////// ////////// //////////
	
	SetDOFSTDAF_WT( cAdjOft );								// 0x0084	adjust offset
	
	UnAdjRes.UlLngVal = MesMSABS1AV();					// 1024 Times Measure average : AD4Z(0x144F)
...
////////// ////////// ////////// ////////// //////////
	...
	ucRegDat = GetDOFSTDAF();
	RegReadA( DRVFC3AF, &ucDRVFC3AF );
	RegReadA( DRVFC4AF, &ucDRVFC4AF );
...
////////// ////////// ////////// ////////// //////////
	...
	if( cAdjOft >= (char)0x20 || cAdjOft < (char)0x00 ){
		usErrFlg = EXE_AFOFF;

	}
	
	if( UnAdjRes.SfFltVal > ( UnAdjMin.SfFltVal + 0.02f ) ){
		usErrFlg = EXE_AFOFF;

	}
	
	StAdjPar.StAfOff.UcAfOff = ucRegDat ;
...
////////// ////////// ////////// ////////// //////////
	...
	for( ucCon = 0; ucCon <= 16 ; ucCon++ ){
		...
		SetDOFSTDAF_WT( ucOff );							// 0x0084	adjust offset
	...
	for( ucCon = 0; ucCon <= 16 ; ucCon++ ){
		if( ucCon == 16 ){
			...
			ucOff = (unsigned char)( 0x02 * ucCon );
		}
		SetDOFSTDAF_WT( ucOff );							// 0x0084	adjust offset
...
////////// ////////// ////////// ////////// //////////
	...
	RegWriteA( OPGSEL2, ucOPGSEL2 );					// 0x0255
	RegWriteA( VGA_SW1, ucVGA_SW1 );					// 0x025B
	RamWrite32A( DAZHLO, ulDAZHLO );					// 0x1529
	RamWriteA( TREG_H,	usTREG_HL ) ;					// TREG_H(0x0380) - TREG_L(0x0381)
	if( usErrFlg == EXE_AFOFF ){
		SetDOFSTDAF_WT( ucDOFSTDAF );
	}
	else{
		SetDOFSTDAF_WT( ucRegDat );
	}
...
unsigned long MesMSABS1AV( void )
{
	...
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	GenMes( AD4Z, 0 );								// 1024 Times Measure average : AD4Z(0x144F)
	RamRead32A( MSABS1AV, &(ulRamDat) ) ;			// 0x1041
	
	return( ulRamDat ) ;
}

//********************************************************************************
// Function Name 	: GetDOFSTDAF
// Retun Value		: NON
// Argment Value	: NON
// Explanation		: 
// History			: First edition 						2014.07.11 T.Tokoro
...
{
	unsigned char	ucDRVFC3AF, ucDRVFC4AF, ucRegDat;
...
void	SetDOFSTDAF( unsigned char ucSetDat )
{
	...
	if( UcCvrCod == CVER122 ){
		...
		RegWriteA( DRVFC4AF, ( ucSetDat & 0x18 ) << 3 );
		RegWriteA( DRVFC3AF, ( ucRegDat & 0x70 ) | ( ucSetDat & 0x07 ) );
	}
	else{
		RegWriteA( DRVFC4AF, ucSetDat << 3 );
	}
/* 20141113 Miyake*/
//	WitTim( AFMID_TIM );							// 
}
